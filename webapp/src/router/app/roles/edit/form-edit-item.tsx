...
export function FormEditItem({ item, currentCapabilities }: FormEditItemProps) {
    const navigate = useNavigate();

    const [modules, setModules] = useState<Module[]>([]);
    const [roleName, setRoleName] = useState<string>(item.role_name);
    const [capabilities, setCapabilities] = useState<string[]>(currentCapabilities);
    const [isFormLoading, setIsFormLoading] = useState<boolean>(true);
    const [error, setError] = useState<string>("");

    // load role capabilities
    const { getRoleCapabilities, updateRole, refetchRoles, refetchRole } = useRole();
    ...
    const saveRole = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // refetch roles
            refetchRoles();
            refetchRole(item.role_id);

            // show success message
            toast.success("Role has been uupdated");

            // navigate to list roles page
            navigate({ to: `/app/roles` });
        } catch (error) {
            console.error("Update role error:", error);
            setError(getErrorMessage(error) || "An unexpected error occurred during update role");
        } finally {
            setIsFormLoading(false);
        }
    }, [item.role_id, roleName, capabilities, isFormLoading, updateRole, refetchRole, refetchRoles]);
